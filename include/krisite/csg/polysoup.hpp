...
struct Poly {
    ...
    /// **★ この多角形が属する八分木の葉の【深度】**（`SPEC-phase5.md` §5.10.6）。
    ///
    /// **`kNoCellDepth` は「不明」**（`from_mesh` 直後のスープ）。
    ///
    /// > **なぜ深度だけで足りるか。**
    /// > `aabb` は葉の箱に含まれるので、`aabb.lo` を深度 $d$ の格子で床関数にかければ
    /// > **葉の添字が復元できます**（`aabb.lo` は $[\text{cell.lo}, \text{cell.hi})$ に居ます）。
    ///
    /// > **なぜ別に持つ必要があるか。**
    /// > **以前は `aabb` にセル箱をそのまま入れており、A-3 の索引が
    /// > 「箱の辺の長さ」から深度を逆算していました**（`tjunction.hpp`）。
    /// > **箱を狭めると逆算できません。**
    /// > **箱に「割り当ての範囲」と「葉の符号」の 2 つの役目を持たせていたのが誤りでした。**
    std::uint8_t cell_depth = octree::kNoCellDepth;
};
...
struct Indicator {
    ...
    /// **旧定義（$w \ne 0$）に戻す**（`SPEC-phase3.md` §5.2.0）。**既定は偽。**
    ///
    /// **正しい定義は $\text{内側}_i \iff w_i > 0$ です。**
    ///
    /// > **自己交差した閉曲面の巻き数は、向きに応じて正にも負にもなります。**
    /// > **立体の内部に対応するのは、巻き数が正の領域です。**
    /// > **負の領域は「面が裏返っている部分」であって、立体の内部ではありません。**
    ///
    /// **初版は $w_i \ne 0$ と定めていました。誤りです**（Phase 5 で実データから判明）。
    /// **巻き数が負にならない入力では両者が一致する**ので、Phase 0〜4 のコーパスでも
    /// CP1（自己交差なし）でも問題が出ませんでした。
    ///
    /// **この旗はケース 27c（対照）のためだけに残しています。**
    /// 「巻き数が負にならない入力では両定義が一致する」ことを検査すると、
