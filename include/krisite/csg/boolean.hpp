...
struct BoolOptions {
    ...
    /// **★ O3: 切る三角形が触れない断片は切らない**（`SPEC-phase5.md` §5.10.14.24。**既定 0 = 切らない判定をしない**）。
    ///
    /// | 値 | 判定 |
    /// |---|---|
    /// | 0 | 従来（セル内の候補平面すべてで切る） |
    /// | 1 | **安い前判定だけ**: 多角形の箱 ∩ セルの箱（整数）が、切る三角形の箱と交わらなければ飛ばす |
    /// | 2 | 1 に加えて、交わるときは断片の頂点と三角形の箱の厳密な比較（Phase 3 の実験と同じ）で飛ばす |
    ///
    /// **既定にするには条件 1（`region_key` の証明かコーパスケース）が要ります。それまで 0。**
    int bsp_skip_disjoint = 0;
