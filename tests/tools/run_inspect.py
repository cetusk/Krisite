...
**既存の監督（`run_gmp_diag.py`）を再利用します。** 監督は作り直しません。
  ...
  * 計算＋終了猶予 … `--budget`（既定 90 秒）。この時刻に遅くとも `TERM`
  * `TERM` → `KILL` … `--grace`（既定 10 秒）
...
def main(argv=None):
    ...
    t0 = time.monotonic()
    t_term = t0 + a.budget
    t_kill = t_term + a.grace
    print("起動: %s" % " ".join(cmd))
    print("  計算＋終了猶予 %s 秒 / TERM→KILL の猶予 %s 秒 / 回収の枠 %s 秒 / "
          "仮想アドレス空間 %s バイト（%s GiB を切り捨て）"
          % (_fmt(a.budget), _fmt(a.grace), _fmt(a.reap), as_bytes, _fmt(a.as_gib)))
    ...
    if state == sup.DONE:
        ...
        print("  停止制御: **発火**（期限に達したので停止させ、回収しました。"
              "終了値 %s が負ならシグナルで終わったことを表します）" % code)
    else:
