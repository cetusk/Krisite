...
def main(argv=None):
    ...
    for name, budget in STAGES:
        now = time.monotonic()
        # ★★ 【取り下げ】以前ここに「この枝は到達不能」と書きましたが、**誤りです**
        #   （`SPEC-phase5.md` §5.10.14.98）。
        #   **子の完了を確認した時刻と、次の段の起動を判定する時刻を同一視していました。**
        #   その間に、子のログの読込み・出力・flush・成果物の確認が入り、
        #   **親が走らない時間（スケジューラ待ち）も入り得ます。**
        #   **完了の確認時に猶予が残っていても、次の判定時に残っている保証はありません。**
        #   **このガードは現行のコードでも必要です。残します。**
        #   **なお、終了値 4 は自己検定で観測できていません（未検定）。**
