...
**包含の導出**（**探針点の近接性にも、体積の一致にも依存しません**。
`docs/SPEC-phase5.md` §5.10.14.91。**旧版の「すぐ内側」に基づく議論は取り下げました**）::

  A0〜A5 と退化面 0 から、B は**互いに交わらない 2 つの連結な閉じた向き付けられた
  埋込み曲面**です。その補空間は**ちょうど 3 領域**で、巻き数は各領域で一定
  （Alexander 双対性）。この実行が標本した点は次の 3 通りです。

  ===================  ==============  ==============
  B についての点        成分 0 の巻き数  成分 1 の巻き数
  ===================  ==============  ==============
  全体 AABB の外部点    0               0
  成分 0 から作った点   1               0
  成分 1 から作った点   1               1
  ...
  **3 通りが全領域を尽くすので、巻き数が (0, 1) の領域はありません。**
  したがって**成分 1 の内部は成分 0 の内部に含まれます**。
  （外部点の成分ごとの値は**導出**です — 点が各成分の外接箱の外にあるため 0。
  ログに残っているのは総和 0 の実測です。）

  A も同じ形です。1 成分なので補空間は 2 領域、外部 0・内部 +1 の標本から、
  **曲面上を除く全体で $w_A \in \{0,1\}$** が出ます。
...
def probe_inside(P, Fc, F_all, others, log, tag):
    r"""成分 `Fc` の内部にある点を 1 つ探し、そこでの巻き数を返す。

    ★★ 【取り下げ】**この関数は「境界のすぐ内側」を保証しません**
    （仕様側の訂正。`docs/SPEC-phase5.md` §5.10.14.91）。
    2 段続けて同値でも、**両方の点が別の境界を越えた同じ領域に入っている**
    可能性を排除できません。他成分の巻き数も比べる形にしても、**近接性の保証には
    なりません**。一般的な「境界のすぐ内側の点を得る方法」としては使えません。

    ★ **包含の結論は、近接性に依存しない形へ差し替えました**（下記「包含の導出」）。
    **この関数が保証するのは「各領域から 1 点ずつ取ること」ではありません。**
    保証するのは、**対象の成分の内部にある点を探し、そこでの巻き数を返すこと**だけです。
    **全 3 領域を覆えたかどうかは、今回の観測結果から外部点と合わせて導いたこと**で、
    この関数の性質ではありません。
    ログに残る「すぐ内側」という語は、この実行が書いた文字列をそのまま保っています
    （証拠と食い違わせないため）。**意味は「その成分の内部にある点」です。**

    面の重心から法線方向へ eps だけ寄せ、**この点で使うすべての巻き数**
    （自分の成分・他の各成分・全体）が**2 段続けて同じ値**になるまで eps を半分にします。
    これは**その 2 つの eps で値が一致したという観測**であって、近接性の確認でも、
    **それ以降の eps に対する安定性の保証でもありません。**
