...
void test_guard_detects_undersized() {
    ...
    // **§3.4 の半開区間により、生の数でも増えなくなりました**（SPEC-phase2）。
    //
    // 以前はこの配置で「面が x=0 に乗るぶん重複だけが増える」ので生の数が 48 → 60 に
    // なり、**生の数で判定すると空回りを見逃す**ことの実例になっていました。半開区間では
    // 境界平面に完全に乗る面が上側のセルにしか入らないので、その重複自体が生じません。
    //
    // **それでも判定は正準化後の数で行ってください。** 重複が増える経路（面が複数セルに
    // またがる構成など）は残っており、この配置で消えたのは「セル境界由来の重複」だけです。
    KRI_CHECK_MSG(s3.raw_fragments == s0.raw_fragments,
                  "半開区間なら、この配置では生の断片数も増えないはず（§3.4）");
}
