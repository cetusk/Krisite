...
// ---
//
// ## 対応付けの誤りは【位相でしか】捕まりません（§5.5.1）★
//
// **体積は分裂の誤りを 1 件も検出しません。** 分裂は体積を変えないので当然です（§5.1.3）。
//
// そのうえで、**壊れ方は 2 通りあり、どちらも検査します**（§5.5.1 の訂正）。
//
//   誤った組で分裂が「起きる」    別シートが次数 2 の辺で繋がり成分が併合
//                                 → **$C$ の不変性**
//   誤った組で分裂が「起きない」  頂点まわりがひとつの扇に潰れる
//                                 → **分裂後も非多様体** + **$\Delta E$ の予測と実測の不一致**
//
// **実測（変異 9 = 対応付けを owner に戻す / ケース 16）— 主検出器は深度で入れ替わります。**
//
//   深度 0      分裂が起きない  → 多様体性・$\Delta E$（予測 4 / 実測 0）・$\Delta\chi$・unresolved
//   深度 1〜3   誤って分裂する  → **$C$ の不変性のみ**（$C$ が 4 → 1）
//
// **片方だけでは深度の半分を取り逃がします。** どちらも消さないこと。
//
...
void run_case(const kritest::Case& c) {
    ...
    for (BoolOp op : {BoolOp::Union, BoolOp::Intersection, BoolOp::Difference}) {
        for (unsigned d = 0; d <= kMaxDepth; ++d) {
            ...
            // ---- §5.5.1 の C 不変性: 誤った組で「分裂が起きる」側の番人 ----
