...
void test_exit_ordering() {
    ...
    // ★ **機構が動いたことを数えます**（案 A）。
    //
    // > **閾値を 12 から 1 に下げました**（`IMPL-phase5.md` §49）。
    // > **12 では、単独実行でも 3 回に 1 回落ちました**（実測 23 / 19 / **7** 件）。
    // > 最大 3 巡まで繰り返しても足りません。**この数は「複数のワーカーが実際に
    // > 扇の仕事を取ったか」で、取るかどうかは走行時の競合次第**だからです。
    //
    // **1 件でも動けば、並列の経路が生きていることは分かります。**
    // **変異 23 の検出は、下の確定的な番人（扇の順序の反転）が受け持ちます。**
    // あちらはスレッド数にも機械の負荷にも依存せず、1 スレッドで効きます。
    //
    // **0 なら本物の後退**です（並列の経路が一度も踏まれていない）。
    KRI_CHECK_MSG(chances >= 1,
                  "**出口の順序が効く機会が 0 件**（分裂頂点 2 以上 かつ 扇が複数スレッド）"
                  "。並列の経路を一度も踏んでいません" +
                      kritest::pair_msg(1, chances));
