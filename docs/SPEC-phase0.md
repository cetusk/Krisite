...
> - ファイル拡張子: `.kris`
> - マジックバイト: ASCII `"KRIS"` + `uint32` バージョン（点群フェーズで使用）
...
入力座標は**符号付き $b$ ビット**、すなわち $coord \in [-2^{b-1},\ 2^{b-1})$ とする（§2）。
以下、すべて符号付きのビット数（符号ビット込み）。

> **正誤（2026-08 修正）**: 旧版は「絶対値が $2^b$ 未満」と書いていましたが、
> **§2 の訂正が否定している当の記述でした。**
> その前提だと座標差が $b{+}2$ になり、下表の $b{+}1$ と矛盾します。
> 波及すると `side` が $9b{+}28$ となり、**$b{=}26$ で 262 ビット（5 リム）**になって
> 「4 リムに収まる」という判断が逆転します。**実装（`widths.hpp`）は符号付き $b$ ビットが正です。**
...
| $x, y, z$ | Cramer（1 列を **$-d$** ベクトルに置換。§6） | $7b + 14$ |
...
#### intersect3 の置換列は **-d** です ★

平面を $N \cdot x + d = 0$ で保持するので、解くべき連立方程式は

$$N_i \cdot x = -d_i \qquad (i = 1,2,3)$$

です。Cramer の公式で列を置換するとき、**右辺は $-d$** になります。

$$w = \det(N_1, N_2, N_3), \qquad
x = \det\begin{pmatrix} -d_1 & b_1 & c_1 \\ -d_2 & b_2 & c_2 \\ -d_3 & b_3 & c_3 \end{pmatrix}$$

$y$、$z$ も同様に、それぞれ第 2 列・第 3 列を $-d$ で置換します。

> **$d$ をそのまま置換すると、符号が反転した点を返します。** 実害のある誤りです。
>
> **正誤（2026-08 修正）**: この注記は §3.1 を加算形（$d = -N \cdot p_1$）に変更したときに
> 必要になったものですが、**文書に書かれていませんでした。**
> 実装（`geom/plane.hpp`）は `neg(d)` を入れており正しいので、**実装が正、仕様が誤り**でした。

**ビット幅は変わりません。** $-d$ は $d$ と同じ幅です。
