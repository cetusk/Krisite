...
**ブール演算**（500 対中 373 対まで到達、進行中）:

| | |
|---|---|
| 成功 / 失敗 / 停止 | 368 / **5** / 0 |
| 失敗の原因 | 次数 4 の辺に集まる 4 枚が**同じ連結成分**に属する配置（36 三角形に最小化済み） |
| 到達した規模 | **12,096 三角形**（CP1 の時点では 8,528 で資源上限に当たっていました） |

**CP1 は 336 対で止まっていました** — 17,138 三角形の対で 30.7 GB に達したためです。
下記の 2 つの改善（メモ化の撤去、レイキャストの加速）で**同じ場所を通過できるように
なりました。** 現在のピークは 17 GB 程度で、**OOM せずに完走**しています。
