...
**説明変数を入力 $n$ から出力の多角形数 $P$ に変えると、2 つの関係に分解できました。**
...
| 成分 | 何が決めるか | 状態 |
|---|---|---|
| $n \to P$ | arrangement の出力量（空間分割の深度） | **実質すでに最適**（下記） |
| $P \to$ 時間 | 分類（大域レイキャスト） | **ここが残る** |
...
$P^{1.31}$ の $0.31$ の大部分が消えます。** Krisite は既に八分木を持っているので、
新しい述語も記号的摂動も要りません。
