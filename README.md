...
**ブール演算**（500 対中 449 対まで到達）:

| | |
|---|---|
| 成功 / 失敗 / 停止 | 442 / **7** / 1 |
| 失敗の原因 | 次数 4 の辺に集まる 4 枚が**同じ連結成分**に属する配置（36 三角形に最小化済み） |
| 到達した規模 | **23,586 三角形**（CP1 の時点では 8,528 で資源上限に当たっていました） |

**CP1 は 336 対で止まっていました** — 17,138 三角形の対で 30.7 GB に達したためです。
下記の改善で**同じ場所を通過できるようになりました**（その対はいま 46 秒で完走します）。

> **これは性能目標ではありません。** Phase 5 の性能目標はまだ設定していません（CP4 の課題）。
> **実データで何が起きたか**の記録です。

**失敗 7 件の形は一様です**（6 件を確認済み。1 件は未確認）。

| | |
|---|---|
| 不足辺 | **0**（面は欠けていない） |
| 最大辺次数 | **4**（5 以上は 1 本もない） |
| 余分辺 | `unresolved` とほぼ一致 |

**面が欠けているのではなく、接触が残っています。**
